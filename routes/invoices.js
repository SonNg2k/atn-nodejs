...
router.post("/", populate.any(), async (req, res, next) => {
    var invoice = req.body.invoice,
        invoiceItem = req.body.item
    try {
        invoiceItem = await InvoiceItem.create(invoiceItem) // First try to create an invoice
        ...
        invoice.invoiceItem = invoiceItem._id

        // Then create the invoice...
        invoice = await Invoice.create(invoice)
        // Populate data to the properties of type mongoose.Schema.Types.ObjectId...
        invoice = await invoice.populate("customer", "-invoices").populate("invoiceItem").execPopulate()

        // Find the toy included in the invoice...
        var includedToy = await Toy.findById(req.body.item._toyID, "name").lean()

        // Then add the name and price of the toy...
        invoice.invoiceItem._doc.name = includedToy.name; // ._doc is the actual data returned to client
